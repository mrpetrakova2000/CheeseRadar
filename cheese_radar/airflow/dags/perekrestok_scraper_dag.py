...
from scraper_tasks import scrape_perekrestok
...
def scrape_perekrestok_with_check():
    """Функция скрапинга Перекрестка"""
    logger.info("Начинается скрапинг Перекрестка")

    result = scrape_perekrestok()
    ...
    if isinstance(result, dict):
        ...
        if new_products_saved == 0:
            error_msg = (
                f"Скрапинг Перекрестка завершился с ошибкой: сохранено 0 новых продуктов. "
        ...
        logger.info(f" Скрапинг Перекрестка завершен успешно")
...
with DAG(
        ...
        description='Скрапинг магазина Перекресток',
        schedule_interval='0 6,18 * * *',
        catchup=False,
        tags=['scraping', 'perekrestok'],
) as dag:
    start = EmptyOperator(task_id='start')

    scrape_task = PythonOperator(
        task_id='scrape_perekrestok_data',
        python_callable=scrape_perekrestok_with_check,
