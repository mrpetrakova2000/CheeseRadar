    ...
    def select_from_mongo(**context) -> Dict[str, Any]:
        ...
        try:
            ...
            if load_type == "latest":
                ...
                query["scraped_at"] = {"$gte": filter_time.strftime("%Y-%m-%d %H:%M:%S")}
                logger.info(f"Фильтр по времени: >= {filter_time.strftime('%Y-%m-%d %H:%M:%S')}")
            ...
            logger.info(
                f"Параметры запроса: load_type={load_type}, store_filter={store_filter}, "
